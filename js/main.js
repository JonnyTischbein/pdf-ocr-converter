...
function process_pdf_preview(file) {
    ...
    if(mime_types.indexOf(file.type) == -1) {
        alert('Fehler: Falscher Dateityp - ' + file.type);
    ...
    if(file.size > 50*1024*1024) {
        alert('Fehler: Die Datei ist größer als 50MB');
        document.querySelector("#pdf-loader").style.display = 'none';
        document.querySelector("#pdf-preview-headline").style.display = 'none';
        return;
    }
...
document.querySelector("#upload-button").addEventListener('click', function(e) {
    ...
    if (droppedFiles){
        formData.append('pdf', droppedFiles[0]);
    }
    else{
...
document.querySelector("#download-pdf").addEventListener('click', function(e) {
    ...
    window.location.href = 'download_pdf.php?pdf=' + file["name"];
});
...
if (isAdvancedUpload){
    $("#dropbox").on('drag dragstart dragend dragover dragenter dragleave drop', function(e) {
      ...
      .on('dragover dragenter', function() {
        $("#dropbox").css("backgroundColor", "#fffeee");
      })
      .on('dragleave dragend drop', function() {
          $("#dropbox").css("backgroundColor", "lightgrey");
      })
      .on('drop', function(e) {
        droppedFiles = e.originalEvent.dataTransfer.files;
        process_pdf_preview(droppedFiles[0]);
        $("#dropbox").hide();
      });
    console.log("Supporting Drag and Drop!");
}
else {
    $("#dropupload-text").hide();
    console.log("NOT supporting drag and drop :(");
}